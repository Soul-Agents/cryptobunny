...
def answer_tweet_with_context_tool(tweet_id: str, message: str) -> str:
    """Search for relevant context and reply to a tweet using that context."""
    try:
        # First get the tweet we're replying to using the existing answer tool
        tweet_content = message  # This should ideally get the original tweet content
        
        # Search for relevant context using the retriever
        relevant_docs = retriever.invoke(tweet_content)
        
        # Extract context from the retrieved documents
        context = "\n".join([doc.page_content for doc in relevant_docs]) if relevant_docs else ""
        
        # Use the context to enhance the reply
        enhanced_message = f"{message}\n\nContext: {context}"
        
        # Use the existing answer tool to post the reply
        result = answer_tool._run(tweet_id=tweet_id, message=enhanced_message)
        return f"Replied to tweet {tweet_id} with context-enhanced message: {enhanced_message}"
    except Exception as e:
...
answer_tweet_with_vector_db_tweet = StructuredTool.from_function(
    func=answer_tweet_with_context_tool,
    name="answer_with_context",
    description="Reply to a tweet with additional context from the vector database"
)
...
tools = [
    ...
    answer_tweet_with_vector_db_tweet
]
#endregion
        ...
        (
            ...
            f"""
                ...
                **Tools:**
                ...
                - **Objective:** Use this tool when you need to provide an informed reply using historical context from our vector database. Perfect for deep discussions or when referencing past conversations and trends.
                - **Note:** This is your secret weapon for showing expertise - it helps you recall relevant past discussions and market insights.
