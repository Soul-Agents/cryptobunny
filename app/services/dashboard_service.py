...
class DashboardService:
    ...
    @staticmethod
    def get_dashboard_data(client_id: str) -> Tuple[bool, Dict[str, Any]]:
        """
        Get dashboard data for a client
        
        Args:
            client_id: Client ID to get dashboard data for
            
        Returns:
            Tuple of (success, dashboard_data)
        """
        try:
            db = get_db()
    ...
    @staticmethod
    def get_stats(client_id: str) -> Tuple[bool, Dict[str, Any]]:
        """
        Get usage statistics for a client
        
        Args:
            client_id: Client ID to get statistics for
            
        Returns:
            Tuple of (success, stats_data)
        """
        try:
            db = get_db()
            auth_data = db.get_twitter_auth(client_id)
    ...
    @staticmethod
    def get_recent_activity(client_id: str, limit: int = 10) -> Tuple[bool, Dict[str, Any]]:
        """
        Get recent activity for all agents owned by a client
        
        Args:
            client_id: Client ID to get activity for
            limit: Maximum number of items to return
            
        Returns:
            Tuple of (success, activity_data)
        """
        try:
            db = get_db()
            auth_data = db.get_twitter_auth(client_id)
